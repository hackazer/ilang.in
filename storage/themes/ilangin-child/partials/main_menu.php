<ul class="navbar-nav ms-lg-auto">
    ...
    <li class="nav-item nav-item-spaced dropdown dropdown-animate" data-toggle="hover">
        <a class="nav-link" data-bs-toggle="dropdown" href="#" aria-haspopup="true" aria-expanded="false"><?php ee('Solutions') ?></a>
...
<ul class="navbar-nav align-items-lg-center d-none d-lg-flex ms-lg-auto">
    <?php if(\Core\Auth::logged()): ?>    
        ...
        <li class="nav-item dropdown dropdown-animate">
            ...
            <div class="dropdown-menu dropdown-menu-sm dropdown-menu-end dropdown-menu-arrow p-3">
    ...
    <li class="nav-item">
        <?php if(!\Core\Auth::logged()): ?>
            <?php if(config("user") && !config("private") && !config("maintenance")): ?>
                <a href="<?php echo route('register') ?>" class="btn btn-sm btn-success btn-icon ms-3">
...
<?php if(config("user") && !config("private") && !config("maintenance")): ?>
    <div class="d-lg-none px-4 text-center">    
        <?php if(\Core\Auth::logged()): ?>
            <a href="<?php echo route('dashboard') ?>" class="btn w-100 btn-sm btn-success"><?php ee('Dashboard') ?></a>
        <?php else: ?>
            <div class="d-flex">
                <div class="w-50 me-1">
                    <a href="<?php echo route('login') ?>" class="btn w-100 btn-sm btn-primary"><?php ee('Login') ?></a>
                </div>
            <?php if(config("user") && !config("private") && !config("maintenance")): ?>
                <div class="w-50 ms-1">
                    <a href="<?php echo route('register') ?>" class="btn w-100 btn-sm btn-primary"><?php ee('Get Started') ?></a>
                </div>
            <?php endif ?> 
            </div>               
        <?php endif ?>          
